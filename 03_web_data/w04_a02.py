...
# will use urllib to read the HTML from the data files below, extract the href= values from the anchor tags, scan for a
# tag that is in a particular position relative to the first name in the list, follow that link and repeat the process
...
# Actual problem: Start at: http://py4e-data.dr-chuck.net/known_by_Cruiz.html
...
# these attempts to make it a little harder to complete the assignment without writing a Python program. But that is
# not the point. The point is to write a clever Python program to solve the program.
#
# Sample execution
#
...
# # FOR SAMPLE TEST
# url = 'http://py4e-data.dr-chuck.net/known_by_Fikret.html' # url for sample data
# iterations = 4
# position = 3 - 1 # 3rd name, 2nd position in zero-based list

# # FOR ACTUAL TEST
url = 'http://py4e-data.dr-chuck.net/known_by_Cruiz.html' # url for actual data
iterations = 7  # follow the last link to get the name of the person
position = 18 - 1  # 3rd name, 2nd position in zero-based list
...
# follow the last link and fetch the name
html = urllib.request.urlopen(url).read()
soup = BeautifulSoup(html, "html.parser")
title = soup('title')[0].string
print(re.findall('People that (\S+) knows', title)[0])
